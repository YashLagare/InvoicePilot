...
export async function GET(
  ...
  data.items.forEach((item) => {
    ...
    pdf.text(descriptionLines, 20, startY);
    pdf.text(item.quantity.toString(), 100, startY);
    pdf.text(formatPDFCurrency(item.rate, data.currency), 130, startY);
    pdf.text(
      formatPDFCurrency(item.quantity * item.rate, data.currency),
      160,
      startY,
    );
    startY +=
      10 +
      (descriptionLines.length > 1 ? (descriptionLines.length - 1) * 4 : 0);
  });

  //total section
  pdf.line(20, startY + 5, 190, startY + 5);
  pdf.setFont("helvetica", "bold");
  pdf.text(`Total (${data.currency})`, 130, startY + 15);
  pdf.text(formatPDFCurrency(data.total, data.currency), 160, startY + 15);

  //optional note
  if (data.note) {
    pdf.setFont("helvetica", "normal");
    pdf.setFontSize(10);
    pdf.text("Note:", 20, startY + 30);
    const splitNote = pdf.splitTextToSize(data.note, 170);
    pdf.text(splitNote, 20, startY + 40);
  }
