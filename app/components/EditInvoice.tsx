...
const EditInvoicePage = ({ data, clients = [], companyProfile }: EditModeData) => {
  ...
  const [invoicePrefix, setInvoicePrefix] = useState(data.invoicePrefix || "INV");
  ...
  // Tax & Discounts
  const [taxRate, setTaxRate] = useState(data.taxRate || 0);
  const [discountRate, setDiscountRate] = useState(data.discountRate || 0);
  const [shippingAmount, setShippingAmount] = useState(data.shippingAmount || 0);
  const [note, setNote] = useState(data.note || "");
  const [invoiceName, setInvoiceName] = useState(data.invoiceName || "");
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 p-4 lg:p-6">
      ...
      <div className="lg:col-span-7">
        <form id={form.id} action={action} onSubmit={form.onSubmit} noValidate>
          ...
          <div className="flex items-center justify-between mb-6">
            <div>
              <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-100">Edit Invoice</h1>
              <p className="text-sm text-slate-500">Update invoice details, items, tax, and notes.</p>
            </div>
            <Badge variant={data.status === "PAID" ? "default" : "outline"} className={data.status === "PAID" ? "bg-green-600" : ""}>
              {data.status}
            </Badge>
          </div>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm p-6 mb-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <Select name={fields.dueDate.name} defaultValue={data.dueDate || "0"}>
                  <SelectTrigger className="h-10 bg-slate-50 dark:bg-slate-950 border-slate-200 dark:border-slate-800 rounded-xl text-sm">
